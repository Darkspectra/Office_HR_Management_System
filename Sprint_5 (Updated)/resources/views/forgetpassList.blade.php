	...
	<section id="section1">
		...
		<table border='1' width="300px" style="margin-left: 10%; margin-right: 10%; margin-top: 50px; margin-bottom: 50px; text-align: center; background: #66b3ff;">
			<tr class='row' style="padding: 5px;">
				...
				<th class='deli'> Password </th>
				<th colspan="3" class='deli'> Action </th>
			</tr>

            @foreach($members as $member)
            <tr class='row' style="padding: 5px;">
                <td class='col-md-3'> {{$member['ID_no']}} </td>
                <td class='col-md-3'> {{$member['Email']}} </td>
                <td class='col-md-3'> {{$member['Password']}} </td>
                <td class='del'> <a href={{"forgetPass/".$member['id']}} data-toggle="tooltip" data-placement="bottom" title="update">Update</a></td>
            </tr>
            @endforeach
